...
namespace Ordering.API.EventBusConsumer
{
    public class BasketOrderingConsumerV2 : IConsumer<BasketCheckoutEventV2>
    {
        ...
        public BasketOrderingConsumerV2(ILogger<BasketOrderingConsumerV2> logger, IMapper mapper, IMediator mediator)
        {
            _mediator = mediator;
            _mapper = mapper;
            _logger = logger;
        ...
        {
            using var scope = _logger.BeginScope("Consuming basket Checkout Event v2 for {correlationId}", context.Message.CorrelationId);
